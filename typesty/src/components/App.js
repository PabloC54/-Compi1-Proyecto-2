...
import { interpret, getPrinted } from '../Analyzer/interpreter'
...
const App = () => {
  ...
  const handleCompile = () => {
    ...
    if (parsed.errors) {
      handleLog('Se encontraron errores léxicos o semánticos. Consulte el reporte de errores')
      return
    }

    interpret(parsed)

    const to_print = getPrinted()
    console.log(to_print)
    to_print.forEach((msg) => {
      handleLog(msg)
    })
  }
